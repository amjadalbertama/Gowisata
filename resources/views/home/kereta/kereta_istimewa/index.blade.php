            ...
            <div class="container">
                <div class="row">
                    <div class="col-9 pt-2"><br>
                        <h1 id="title_kai_istimewa" class="h5 font-weight-normal text-center text-uppercase text-colors-on d-none"> <b>Pilih Gerbong Kereta Istimewa</b></h5>
                                    <hr>
                        <!-- <div class="card shadow p-3 mb-5 bg-body rounded"> -->
                            <div class="card-body">
                                <div class="" id="list_krl_ist">
                                       
                                </div>
                                <br><br>
                                <div class="row col-12 text-center">
                                        <div class="col-4"></div>
                                        <div id="btn_nx_kaiist"></div>  
                                </div>
                            </div>
                        <!-- </div> -->
                    </div>
...
<script>
    ...
    var scadule = JSON.parse(localStorage.getItem('data_scadule_istimewa'));
    ...
    var token = JSON.parse(localStorage.getItem('data_token'));
        ...
        var url = fetch(mainurl +'train/list_wagon?wagon_id=&wagon_type=Istimewa',{
        method: 'GET',
        headers: {
        ...
        $("#title_kai_istimewa").removeClass('d-none');
            ...
            for (var i = 0; i < train.length; i++){
                ...
                $.LoadingOverlay("hide")
                    ...
                    <div class="card shadow p-3 mb-2 bg-body rounded">
                        ...
                        <input type="checkbox" class="custom-control-input check_box_kai_wisata" id="`+train[i].id+`" value="`+train[i].id+`" name="kai_wisata_check" checked disabled> 
                            <label for="`+train[i].id +`" class="custom-control-label col-12">
                                 <div class="row">
                                    ...
                                    <div class="col-8">
                                        ...
                                        <input id="name_list_wagon_ist`+train[i].id +`" value="`+train[i].name+`" hidden></input>
                                        <input id="capacit_list_wagon_ist`+train[i].id +`" value="`+train[i].capacity+`" hidden></input>
                                    </div> 
                                </div>
                            </label>
                        </div>
                    </div> `;
                    $("#list_krl_ist").append(setPaxLght);
                }
                ...
                var btn_selct_krl = `<button class="btn col-4" id="" style="background-image: linear-gradient(red, rgb(251, 140, 1));" onclick="setIdKaiist()"><b class="text-white">selanjutnya</b></button>`
                $("#btn_nx_kaiist").replaceWith(btn_selct_krl);
                //  var checkboxes = document.getElementsByName('kai_wisata_check');
                //  console.log(checkboxes.length)  
            });   

            function setIdKaiist(){
                            ...
                            <div class="modal-dialog modal-sm modal-dialog-scrollable">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-light" data-dismiss="modal"><i class="fa fa-times mr-1" onclick="closeModalvalidateIst()"></i>Cancel</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                // name_wagon = []
                    for (var i = 0; i < kai_val.length; i++) {
                        var names = $("#name_list_wagon_ist" + kai_val[i].value).val()
                        var capa_wagon = $("#capacit_list_wagon_ist" + kai_val[i].value).val()
                    ...
                    // console.log(idwagon);
                    localStorage.setItem("id_kai_istimewa", JSON.stringify(idwagon));
                    chooseDataContact()
                }
            }

            function closeModalvalidateIst() {
            ...
            function chooseDataContact(){
                window.location.href = "/kaiist/list/contact";
            }
