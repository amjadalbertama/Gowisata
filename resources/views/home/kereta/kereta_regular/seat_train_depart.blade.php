                    ...
                    <div class="col-12 pt-2">
                        <div class="card shadow p-3 mb-5 bg-body rounded">
                            <div class="card-body ">
                                <div class="row">
                                    <div class="col-12">
                                        <h1 class="h5 font-weight-normal text-center text-uppercase text-colors-on"> <b>Pilih Kursi Kereta Keberangkatan</b> </h5>
                                        <hr>
                                        <br>
                                        <div class="row">
                                            <div class="col-1">
                                            ...
                                            <div class="col-4">
                                                ...
                                                <div class="card shadow-sm col-12">
                                                    <div class="card-body">
                                                    <h5 class="text-colors-on"><b> Penumpang dan gerbong </b></h5>
                                                        <hr>
                                                        <div class="row">
                                                            <div class="form-group col-8">
                                                                <label for="">Pilih Penumpang : </label>
                                                                <select class="form-control form-control-sm" id="list_pax" name="" value="">
                                        ...
                                        <div class="col-6 pt-2">
                                            <div class="row">
                                                ...
                                                <select class="form-control form-control-sm col-7" id="list_gerbong" name="" value=""></select> 
                                            </div><br>
                                            <div id="map_seat" class="col-12 d-none" style=" border: 4px solid; border-radius: 5px; border-color: rgb(251, 140, 1); margin-left:30px; overflow-x: scroll; height: 800px;">
                                                <div class="row" id="main" style="margin: auto; display:flex; align-items:center;"> 
                                ...
                                <div class="row col-12 pt-5">
                                    ...
                                    <div class="col-3">
                                        <button class="btn btn-block  md-auto top-100 start-50 main-color" onclick="toReturnTrainSeat()"><b class="text-colors-off">Pilih Kursi Kepulangan</b> </button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    var url = fetch(mainurl +'train/booking_detail',{
        ...
        body: JSON.stringify({
            ...
            "booking_code_return": code_retrn,
            
        }),
        
        }).then((response) => response.json()).then((responseData) => {
        var train = responseData.data.departure;
        console.log(train)
    ...
    var url = fetch(mainurl +'train/seat_map',{
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
            ...
            let seat_gerbong = responseData.data.departure.seat_map ;
            
            for(let train = 0; train < seat_gerbong.length; train++){
                var no = train + 1
                    setPaxLght = '<option value="'+ no +'"> Gerbong-'+ no +' '+ seat_gerbong[train][0]+'</option>';
                    $("#list_gerbong").append(setPaxLght);
            }  
            const gerbong = $("#list_gerbong").val();
            
            setSeatKaireg(seat_gerbong, gerbong)
        });

        function setSeatKaireg(seats, wagons){
            const seatMap = seats.find(seat => seat[1] == wagons);
            const maxRow = Math.max(...seatMap[2].map(seat => seat[0]));
            const maxCol = Math.max(...seatMap[2].map(seat => seat[1]));
            const table = document.createElement("table");
            const div = document.createElement("div");
            div.innerHTML = '<h5><b style="margin-left: 40%;">Gerbong-'+seatMap[1]+' '+seatMap[0]+'</b> <br></h5>';
            // table.createCaption().innerHTML = '<hr>' ;
            table.setAttribute('class', 'table w-auto sm-1');
            table.style.border = "none";
            table.setAttribute('id', seatMap[0]+seatMap[1]);
            ...
            for (let row = 1; row <= maxRow; row++) {
              ...
              for (let col = 1; col <= maxCol; col++) {
                ...
                const seat = seatMap[2].find(seat => seat[0] == row && seat[1] == col);
                let btn = document.createElement('button');
                btn.className = 'btn btn-primary w-100';
                btn.innerHTML = seat ? seat[0]+seat[3] : 'X';
                btn.onclick = function(){
                    changeSeatKai(btn.innerHTML , seatMap[1], seatMap[0])
                    }
                if (seat && (seat[5] === 1)) {
                  btn.disabled = true;
                  td.appendChild(btn);
                  ...
                  for(let i = 0; i < seat_data.length; i++){
                    if(seatMap[1] == seatPaxOn()[i]['gerbong']){
        ...
        $("#list_gerbong").on("change", function(){
            $.LoadingOverlay("show")
            $("#main").empty()
            const gerbong = $(this).val();
            var url = fetch(mainurl +'train/seat_map',{
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
   ...
   function okChangeSeat(){
        ...
        var token = JSON.parse(localStorage.getItem('data_token'));
        var token_set = token['access_token']
        var url = fetch(mainurl + 'train/change_seat',{
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
   ...
   function simpanAbayar(){
    ...
    function toReturnTrainSeat(){
        window.location.href = "/setSeatkrlregReturn";
    }
</script>
@endpush